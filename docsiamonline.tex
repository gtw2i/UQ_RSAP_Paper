...
\begin{itemize}
\item \texttt{siamonline171218.cls} (required): Main SIAM online-only \LaTeX\ class file.
...
\end{itemize}
To use these files, put \texttt{siamonline171218.cls} and
...
\begin{example}[label={ex:outline},listing only,%
  listing options={style=siamlatex,{morekeywords=[1]{maketitle},
      morekeywords=[2]{siamonline171218}},}]%
  {Document outline}
\documentclass{siamonline171218}
% Preamble: packages and macro definitions go here.
% Preamble: define title, authors, headers here.
\begin{document}
\maketitle
...
There are times when the author may want to specify
...
\documentclass[supplement]{siamonline171218}
\input{ex_shared}
...
the SIAM online-only \LaTeX\@ and \BibTeX\ classes:
\begin{itemize}
\item Change in file names: \texttt{siamonline1116.cls} is replaced by
  \texttt{siamonline171218.cls}, and the \texttt{siam.bst} bibliography style
...
\item New command for setting headers: \code{\headings}; see
  \cref{sec:front}. 
\item New command for funding information: \code{\funding}; see
  \cref{sec:front}. 
...
In past versions, 
the SIAM online-only \LaTeX\ class defined the following macros:
\code{\const},
\code{\diag},
\code{\grad},
\code{\Range},
...
\cref{ex:numbering} as documentclass options (e.g., \code{\documentclass[review,onefignum,onetabnum]} \code{{siamonline171218}}).
