...
%=======
			\STATE Initialize chain: $\theta_0^{i,j} \sim U([-L,L]^M)$
			\STATE Calculate $\theta_n^{i,j}$ for $n = 1, ..., N_{step}$ steps using either Metropolis or RSAP with initial state $\theta_0^{i,j}$ and fixed proposal covariance matrix $\Sigma^i = \sigma^i I$
