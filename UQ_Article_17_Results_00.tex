...
We now move on to present results from our optimization-inspired experiments. These used statistics from large ensembles of chains to provide a systematic way of comparing the short-term (i.e., burn-in) performance of RSAP and Metropolis over different proposal distributions. This is useful because it allows us to compare both methods in their optimal context, since sweeping through a large range of widths is guaranteed to contain a near-optimal value. As we shall soon see, RSAP consistently outperforms Metropolis for most functions, dimensionalities, and proposal widths (confirm???).
...
\end{equation}
over a domain defined by the $M$-dimensional cube $[-L,L]^M$. We sweep through $N_{width}$ proposal widths at equally-spaced intervals. We say widths rather than covariance matrices since because the Ackley function is isotropic along the parameter axes, there is no need for differing widths for each parameter. For each width, we run $N_{chains}$, each for $N_{steps}$, with initial states uniformly-distributed in the domain. The cumulative probability of convergence to the global maximum is then calculated over the time-steps for each width by determining at which time-step each chain's model error is below the threshold $\varepsilon$.
\begin{algorithm}[h]
\algsetup{
	indent=12pt,
	linenosize=\small,
