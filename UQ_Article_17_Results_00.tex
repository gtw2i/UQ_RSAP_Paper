...
\begin{algorithmic}[1]
	...
	\FOR{ $n = 1$ \TO $N$}
		...
		\hskip\algorithmicindent \hspace{1.20in} $=\textrm{min}\Bigg(1, \dfrac{\ell(y|\theta')p(\theta')} {\ell(y|\theta^{n-1})p(\theta^{n-1})}\Bigg)$
		\end{varwidth}
...
The Metropolis method has several useful properties which contribute its effectiveness. First---interpreting the method similar to an annealing-type optimization method---it can both naively hill-climb and tactically accept poorer states to escape suboptimal local modes. When testing a new candidate state for acceptance, if it has an equal or higher posterior value than the current state, then $\alpha = 1$ and acceptance is guaranteed (hill-climbing). On the other hand, if the posterior is lower, then $1 > \alpha \ge 0$ and the poorer state still has a probability of being accepted. Though poorer states can be accepted, $\alpha$ decreases as the candidate's posterior becomes lower, so very poor candidates will usually be rejected. Counter-intuitively, this ability to accept poorer states is required to guarantee the eventual convergence of Metropolis (as evidenced by the appearance of the acceptance probability in the proof in the following paragraph).
...
\begin{equation}
	...
	\label{Prop1D}
\end{equation}
where $\sigma_t^n \le \sigma_f \le \sigma_w^n$ are the proposal widths and $0 \le p^n_t, p^n_f, p^n_w \le 1$ are the mixing probabilities. Every parameter uses separately adapting wide and thin proposal widths---allowing for independent adaptation of each---as well as its own fixed width (since the different parameters are likely different units/scales).
\begin{figure}[!htb]
	\centering
	...
	\endminipage\hfill %%%%%%%%%%%%%%%%%%%
	\caption{RSAP proposal with $\sigma_t = 0.5, \sigma_f = 1.5, \sigma_w = 2.5, p_t = p_w = p_f = 1/3$.}
\end{figure}
...
\begin{algorithmic}[1]
	...
	\FOR{ $n = 1$ \TO $N$}  % the step number of the system
...
\end{algorithm}
We use a uniform prior over the given domain and a diagonal covariance matrix $\hat \Sigma$ for our fixed Gaussian. We define $N$ as the number of time steps and $M$ as the number of parameters. We also define separate counters $k_{w}^m$, $k_{t}^m$ for every parameter $m = 1, ..., M$. As can clearly be seen, steps 3-20 are the additional adaptive functionality while steps 21-23 are from the standard Metropolis method.


\subsection{Ergodicty of RSAP}
We will now provide some analysis of the method, proving conditions $a$ and $b$ of Theorem 5 from Roberts and Rosenthal \cite{RRAdaptErgo} for our method, thereby showing its ergodicity. Theorem 5 states
\begin{theorem}
	Consider an adaptive MCMC algorithm, on a state space $\mathcal{X}$, with adaptation index $\mathcal{Y}$, so $\pi( \cdot )$ is stationary for each kernel $P_y$ for $y \in \mathcal{Y}$. Assume that:
	\begin{tight_enum}
...
Let $c^{n}_{1, 0, 0, \cdots, i^1_M, i^2_M, i^3_M}$ be the coefficient of this proposal.
Then, using the recurrence relation, we get
\begin{equation}
...
There is also the matter of setting the adaptation parameters to their proper values. Clearly, the optimal values of these parameters are problem-dependent; so we attempted to find a set of reasonable values which improved the performance gain over Metropolis for as many test cases as possible. We thus arrived at maximum scaling magnitudes of $\hat A_{t} = 0.1, \hat A_{w} = 10$, respectively. Being able to both increase and decrease the proposal width by an order of magnitude ensures that the true optimal width is likely contained within the interval $[\hat A_t \sigma_f, \hat A_w \sigma_f]$. Next, concerning the scaling rates, we found that their values were less significant than the magnitudes in determining performance gain. Their values were set to $r_{t} = r_{w} = 0.3$, since this puts the scaling to within approximately 5\% of its maximum effect within 10 rejection steps ($e^{-0.3 \cdot 10} \approx 0.049$). Smaller rate values would require more consecutive rejections to reach the same amount of scaling, thus leaving the more extreme scaling values unused. Lastly, the interval widths $n_1, n_2$ are likely the most problem-dependent parameters since they determine how long RSAP is allowed to be adaptive. As discussed earlier, $n_1 < N$ should be large w.r.t. $N$ while $n_2 \leq N - n_1$ has more freedom. Because of their problem dependence, we will vary them throughout the tests.
...
\end{equation}
where $a = 1, b = 0.3, \theta \in [-1, 1]$. The initial state values were set to $\theta_0 = 0.8, \Delta_0 = 0.08$ and the fixed proposal widths were set to $\sigma_\theta = 0.1, \sigma_\Delta = 0.0008$. As discussed above, the parameters for RSAP were set to $\hat A_{t} = 0.1, \hat A_{w} = 10, r_{t} = 0.3, r_{w} = 0.3, n_1 = 2000, n_2 = 1000$.
\begin{figure}[!htb]
...
\end{equation}
where $a = 0.5, b = 0.15, c = 0.333, d = 8, \theta \in [-1, 1]$. The initial state values were set to $\theta_0 = 0.8, \Delta_0 = 0.08$ and the fixed proposal widths were set to $\sigma_\theta = 0.1, \sigma_\Delta = 0.0008$. As discussed above, the parameters for RSAP were set to $\hat A_{t} = 0.1, \hat A_{w} = 10, r_{t} = 0.3, r_{w} = 0.3, n_1 = 2000, n_2 = 1000$.
\begin{figure}[!htb]
\minipage{0.33\textwidth}
...
Obviously, this test can be easily skewwed by choosing method parameters which work well for RSAP but poor for Metropolis or AM. The tests in the next section provide a more fair comparison, letting us examine the optimal performance of each method.


\subsection{Convergence to global minimum}
We now move on to present results from our optimization-inspired experiments. These use statistics from large ensembles of chains to provide a systematic way of comparing the short-term (i.e., burn-in) performance of Metropolis, Adaptive Metropolis, and RSAP over different proposal widths. This is useful because it allows us to compare both methods in their optimal context, since sweeping through a large range of proposal widths is guaranteed to contain a near-optimal value. As we shall soon see, RSAP outperforms both Metropolis and Adaptive Metropolis for most proposal widths, particularly in higher dimensionalities.
...
\begin{algorithmic}[1]
	\STATE Define the $M$-dimensional parameter vector $\theta^{n,i,j}$ as the $n$-th time-step of the $j$-th chain of the $i$-th proposal width
	\FOR{$i = 1$ \TO $N_{width}$}
...
%=======
			\STATE Initialize chain: $\theta^{0,i,j} \sim U([-L,L]^M)$
			\STATE Calculate $\theta^{0,i,j}$ for $n = 1, ..., N_{step}$ steps using either Metropolis or RSAP with initial state $\theta^{0,i,j}$ and fixed proposal covariance matrix $\Sigma^i = \sigma^i I$
%>>>>>>> b5fcbc9db7f719088b01834d947fbf38cd06bd54
		...
		\STATE Initialize the cumulative distribution: $c^i_0 = 0$ 
			...
			\FOR{$n = 1$ \TO $N_{step}$}
				\IF{\NOT isConv \AND $f(\bar \theta_n^{0,i,j}) \le \varepsilon$}
...
\end{algorithm}
We sweep through $N_{width}$ proposal widths at equally-spaced intervals, i.e., $\sigma^i = i \sigma_{max} / (N_{width}-1)$ where $0 \le i \le N_{width}-1$.  Obviously, since RSAP adapts the proposal width, we will be sweeping through its \textit{fixed} proposal width, letting these values also equal the widths for Metropolis.  We say ``widths" rather than ``covariance matrices" since---for the Ackley function---each parameter axis is scaled equally, thus causing a single width to work for each. For every width value, we run $N_{chains}$, each for $N_{steps}$, with initial states that are uniformly-distributed across the cubical domain. The cumulative probability of convergence to the global minimum of the Ackely function versus the time-step is then calculated for each width by determining at which time-step each chain's Ackley evaluation is below the threshold $\varepsilon$.

Let us note two more considerations before presenting the results. Since we are merely concerned with the short-term behavior of RSAP, the values $n_1,n_2$ play no role, thus placing us in the adaptive regime the entire time. Also, concerning the value of $\Delta$, lower values are superior for fast initial convergence but poor for getting accurate sampling of the posterior in the long-term. RSAP, in particular, prefers a low value of $\Delta$, since doing so causes more rejections to occur, letting RSAP adapt more frequently. Thus, we will restrict ourselves to only small values of $\Delta$.
...
\\
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_01_AdaF.jpg}
\\
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_02_AdaF.jpg}
\endminipage\hfill %%%%%%%%%%%%%%%%%%%
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_00_AM.jpg}
\\
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_01_Diff.jpg}
\\
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_02_Diff.jpg}
\endminipage\hfill %%%%%%%%%%%%%%%%%%%
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_00_AdaT.jpg}
\\
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_01_AdaT.jpg}
\\
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_02_AdaT.jpg}
\endminipage\hfill
...
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_003D_00_AdaT.jpg}
\endminipage\hfill
\caption{Plots of the cumulative convergence of Metropolis (left), Adaptive Metropolis (center), and RSAP (right) performed on a 3D Ackley function. We used $N_{burn} = 250$, $\Delta = 0.01$, 5000 time-steps per chain, 500 chains per width value, and threshold of 1.0 over the domain $[-15.0,15.0]^{3}$.}
\end{figure}
Figure 5 contains the results of a comparison between Metropolis, Adaptive Metropolis, and RSAP on a 3D Ackley function. There are several features which must be discussed. First, we notice that both Metropolis and RSAP have similarly shaped convergence profiles, with the fastest rate of convergence happening near some particular optimal value of the proposal width and a tapering off as the width is increased or decreased. The main differences between the two are 1) that RSAP's optimal width is slightly less than that of Metropolis and, more obviously, 2) that RSAP's convergence rate is significantly higher than Metropolis's for all width values. On the other hand, the convergence profile for AM is quite different, with the transition into the adaptive regime being very clearly visible. Since AM's width (or covariance matrix) should converge to the same value(s) no matter the initial value(s), we see similar performance across a wide range of initial widths. Also, though AM's convergence does have an optimal width value, it is much more forgiving than the other two methods when using too large of a width. However, RSAP is still able to perform well when given widths which are too small for Metropolis or AM. Considering just these small width values, it is reasonable for Metropolis and AM to stagnate on the Ackley function if given too small a width. Each trough in the Ackley function has a width of 1.0 between adjacent modes. This is due to the cos$(2 \pi \theta_m)$ term. Now, for a Gaussian distribution, 99.7\% of samples drawn will be within $3 \sigma$ of the mean. Therefore, letting $1.0 = 3 \sigma$, we see that a width of 0.333 will only draw samples outside of a particular trough 0.3\% of the time (assuming that the mean is approximately at the center of the trough, which is reasonable since it will be attracted to that location). If Metropolis were to use such a small width, it would have a very small chance to escape. Similarly for AM, if its estimation of the optimal width ever became this small, drawing further samples from within the trough would only serve to \textit{decrease} the width estimate even further (AM would have a better chance of escaping such a trough in a low number of dimensions since the coefficient $2.38^2/d > 1$, thus causing the width value to be large than 0.333). Looking back to Figure 5, we see no convergence in Metropolis or AM for width values at or below 0.333, with AM actually performing worse than standard Metropolis. On the other hand, due to RSAP's ability to both grow and shrink the width, it still converges well within this region.
...
Next, in Figure 6, we have a comparison over a 10D Ackley function.
\begin{figure}[!htb]
\minipage{0.333\textwidth}
...
\endminipage\hfill %%%%%%%%%%%%%%%%%%%
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_010D_00_AdaT.jpg}
\endminipage\hfill
\caption{Plots of the cumulative convergence of Metropolis (left), RSAP (center), and their difference (right) performed on a 10D Ackley function. We used $N_{brun} = 2000$, $\Delta = 0.01$, 5000 time-steps per chain, 500 chains per width value, and threshold of 1.0 over the domain $[-15.0,15.0]^{10}$.}
\end{figure}
Note the scale of the axes has changed dramatically from Figure 5, since a 10D space requires more time-steps to explore and the optimal width decreases \cite{SigScale}. In spite of the change of scale, the shape of the three methods' convergence profiles remains similar to their low-dimensional counterparts, with Metropolis and RSAP have a clear optimal width and AM have roughly equal performance across a range of widths. Just as in Figure 5, it is now quite noticable that the optimal width for RSAP is less than that for Metropolis. Also, for most widths, RSAP completely dominates Metropolis. Interestingly, RSAP and AM seem to have complementary convergence regions (at least up to 5000 steps).

Finally, we have a comparison over a 20D Ackley function (note that the axes' scales have changed again).
\begin{figure}[!htb]
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_020D_00_AdaF.jpg}
\endminipage\hfill %%%%%%%%%%%%%%%%%%%
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_020D_00_AM.jpg}
\endminipage\hfill %%%%%%%%%%%%%%%%%%%
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_020D_00_AdaT.jpg}
\endminipage\hfill
\caption{Plots of the cumulative convergence of Metropolis (left), RSAP (center), and their difference (right) performed on a 20D Ackley function. We used $N_{burn} = 5000$, $\Delta = 0.001$, 10000 time-steps per chain, 500 chains per width value, and threshold of 1.0 over the domain $[-10.0,10.0]^{20}$.}
\end{figure}
For this test, we shrunk the domain from $[-15.0,15.0]^{20}$ to $[-10.0,10.0]^{20}$ and decreased $\Delta_0$ by a factor of 10. Though it is difficult to tell from the image, there was zero convergence from any of the Metropolis \textit{or} Adaptive Metropolis chains within the prescribed number of steps. RSAP, on the other hand, still performed quite well, though the range of quickly-converging widths has been greatly compressed. From this plot, it is clear that the convergence rate of Metropolis and AM in high-dimensional spaces pales in comparison to that of RSAP.
...
\section{Conclusions}
From both our analytical and experimental results, it is clear that RSAP deserves a well-earned a place in the vast marketplace of adaptive MCMC methods. Though it performs well in low-dimensions, its true power is fast convergence in high-dimensional, multimodal spaces. The obvious next steps to be taken in our future work is to combine RSAP (or a variant with slightly different adaptation rules or scaling functions) with AM in such a way that we get the fast convergence of RSAP along with the consistent convergence across a wide range of widths of AM. We ucrrently have in mind several techniques for accomplishing this and we hope to test them as soon as possible.
...
\bibitem{AM_RR} Roberts, G. O. and Rosenthal, T. S. 2009. Examples of adaptive MCMC. Journal of Computational and Graphical Statistics 18(2):349-367

\bibitem{SigScale} G. O. Roberts, A. Gelman, and W. R. Gilks (1997). Weak convergence and optimal scaling of random walk Metropolis algorithms. Ann. Appl. Probab. 7, 110-20.
