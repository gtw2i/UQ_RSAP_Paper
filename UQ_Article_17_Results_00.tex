...
\endminipage\hfill %%%%%%%%%%%%%%%%%%%
\minipage{0.333\textwidth}
\includegraphics[width=\linewidth]{UQPlot_ConvWidth_Ackley_020D_00_Diff.jpg}
\endminipage\hfill
\caption{Plots of the cumulative convergence of Metropolis (left), RSAP (center), and their difference (right) performed on a 20D Ackley function. We used $\Delta = 0.001$, 10000 time-steps per chain, 500 chains per width value, and threshold of 1.0 over the domain $[-10.0,10.0]^{20}$.}
\end{figure}
For this test, we shrunk the domain from $[-15.0,15.0]^{20}$ to $[-10.0,10.0]^{20}$ and decreased $\Delta_0$ by a factor of 10. Though it is difficult to tell from the image, there was zero convergence from any of the Metropolis chains.
