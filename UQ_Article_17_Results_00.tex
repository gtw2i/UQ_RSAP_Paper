...
We now move on to present results from our optimization-inspired experiments. These used statistics from large ensembles of chains to provide a systematic way of comparing the short-term performance of RSAP and Metropolis over different proposal distributions. This is useful because it allows us to compare both methods in their optimal context, since sweeping through a large range of widths is guaranteed to contain a near-optimal value. As we shall soon see, RSAP consistently outperforms Metropolis for most functions, dimensionalities, and proposal widths (confirm???).

When performing this test (Algorithm 3.1), we truncate the domain to the $M$-dimensional cube $[-L,L]^M$. We sweep through $N_{width}$ proposal widths at equally-spaced intervals. For each width, we run $N_{chains}$ with initial states uniformly-distributed in the domain. These chains are then run for $N_{step}$ time-steps. The cumulative probability of convergence to the global maximum is then calculated over the time-steps for each width by determining at which time-step each chain's model error is below the threshold $\varepsilon$.
\begin{algorithm}[h]
\algsetup{
	indent=12pt,
	linenosize=\small,
...
\begin{algorithmic}[1]
	\FOR{$i = 1$ \TO $N_{width}$}
		...
		\FOR{$j = 1$ \TO $N_{chain}$} % check for convergence
			\STATE Initialize the cumulative distribution: $c^{i,j}_0 = 0$
			\STATE Initialize convergence check: isConv = false
			\FOR{$n = 1$ \TO $N_{step}$}
				\IF{$f(\theta_n^{i,j}) \le \varepsilon$ \AND \NOT isConv}
					\STATE $c^{i,j}_n = c^{i,j}_{n-1} + 1$
					\STATE isConv = true
				\ELSE
					\STATE $c^{i,j}_n = c^{i,j}_{n-1}$
				\ENDIF
			\ENDFOR
		\ENDFOR
	\ENDFOR
