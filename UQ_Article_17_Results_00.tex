...
\end{figure}
As can be seen, there is efficient mixing between the two modes in the region up until $n = n_1$. Once standard Metropolis begins to overtake the adaptive effects, the chain becomes ``stuck" in whichever mode it was currently in. The miving between the two modes WILL BECOME EQUAL ASYMPTOTICALLY, BUT FOR A SMALL NUMBER OF STEPS, IT IS LIKELY TO RESULT IN UNEQUAL SAMPLING IN THIS FUNCTION...  JW Obviously, however, this test can be easily skewwed by choosing method parameters which work well for RSAP but poor for Metropolis. The tests in the next section provide a fairer comparison, letting us examine the optimal performance of both methods.


\subsection{Convergence to global maximum}
We now move on to present results from our optimization-inspired experiments. These used statistics from large ensembles of chains to provide a systematic way of comparing the short-term (i.e., burn-in) performance of RSAP and Metropolis over different proposal distributions. This is useful because it allows us to compare both methods in their optimal context, since sweeping through a large range of proposal widths is guaranteed to contain a near-optimal value. As we shall soon see, RSAP outperforms Metropolis for most functions, dimensionalities, and proposal widths (confirm???).
...
\end{equation}
over a domain defined by the $M$-dimensional cube $[-L,L]^M$. We sweep through $N_{width}$ proposal widths at equally-spaced intervals.  Obviously, since RSAP adapts the proposal width, we will be altering the \textit{fixed} proposal width, letting these values also be the widths for Metropolis.  We say widths rather than covariance matrices since because the Ackley function is isotropic along the parameter axes, there is no need for differing widths for each parameter. For each width, we run $N_{chains}$, each for $N_{steps}$, with initial states that are uniformly-distributed across the domain. The cumulative probability of convergence to the global maximum is then calculated over the time-steps for each width by determining at which time-step each chain's model error is below the threshold $\varepsilon$.
\begin{algorithm}[h]
\algsetup{
	indent=12pt,
	linenosize=\small,
...
=======
			\STATE Initialize chains: $\theta_0^{i,j} \sim U([-L,L]^M)$
			\STATE Calculate  $\theta_n^{i,j}$ for all n steps in the chain using both Metro/RSAP with initial state $\theta_0^{i,j}$ and fixed proposal covariance matrix $\Sigma^i = \sigma_i * I$
>>>>>>> b5fcbc9db7f719088b01834d947fbf38cd06bd54
		...
		\STATE Initialize the cumulative distribution: $c^i_0 = 0$ 
			...
			\FOR{$n = 1$ \TO $N_{step}$}
				\IF{\NOT isConv \AND $f(\theta_n^{i,j}) \le \varepsilon$}
