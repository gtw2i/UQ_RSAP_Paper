...
Two more considerations before presenting the results. Sinc ewe are merely concerned with the short-term behavior of RSAP, the values $n_1,n_2$ play no role, thus placing in the adaptive regime the entire time. Also, concerning the value of $\Delta$, lower values are superior for fast initial convergence but poor for getting accurate sampling of the posterior in the long-term. RSAP, in particular, prefers a low value of $\Delta$, since doing so causes more rejections to occur, letting RSAP adapt more frequently. Thus, for each test, we will include results using several different values of $\Delta$ so that its effect can be clearly seen.

% IMPORTANT: I think since small $\Delta$ is good for RSAP but bad for getting a realistic posterior distribution (too peaked), we might should reframe the method as adaptive with smooth burn-in and just prove that the burn-in phase leads to ergodicity. Might have to manually change $\Delta$ at the end of the adaptive phase ($n_1+n_2$)
...
Plot discussion: Since the width between adjacent modes in the Ackley function is 1, a proposal width of 0.333 or less will draw samples between the modes 99.7\% or more of the time ($3\sigma$)
